...
def get_users_for_shell(passwd_output: str = PASSWD_OUTPUT,
    ...
    passwd_elements = [line.split(":") for line in passwd_output.splitlines()]
    users_for_shell = [username for (
        username, *_, shell) in passwd_elements if shell.endswith(f"/{grep_shell}")]
    return users_for_shell
