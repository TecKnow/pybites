...
def most_popular_characters(characters=characters, top=5):
    ...
    """
    local_characters = [x._replace(appearances=int(x.appearances)) for x in characters if x.appearances]
...
def max_and_min_years_new_characters(characters=characters):
    ...
    """
    local_characters = [x._replace(year=int(x.year)) for x in characters if x.year]
    characters_by_year = Counter(x.year for x in local_characters).most_common()
    return str(characters_by_year[0][0]), str(characters_by_year[-1][0])
...
def get_percentage_female_characters(characters=characters):
    ...
    """
    local_characters = [x for x in characters if x.sex]
