...
async def show_foods_for_user(request: Request, username: str):
    ...
    foods_for_user = (food_entry for food_entry in food_log.values()
                      if food_entry.user.username == username)
    table_rows = '\n'.join((TABLE_ROW.format(
    ...
    ) for food_entry in foods_for_user))
    response = HTML_TEMPLATE.format(username=username, table_rows=table_rows)
    return response
