...
def _get_vital_dates(composer: str) -> Tuple[date, date]:
    composer_tuple = composers.get(composer)
    if composer_tuple is None:
        raise ValueError(f"unknown composer: {composer}")
    return _get_date(composer_tuple.born), _get_date(composer_tuple.died)
...
def operas_both_at_premiere(guest: str, composer: str) -> Iterator[str]:
    ...
    """
    composer_born, composer_died = _get_vital_dates(composer)
    guest_born, guest_died = _get_vital_dates(guest)
    ...
    for opera in composer_operas:
        ...
        if composer_born <= opera_date <= composer_died and guest_born <= opera_date <= guest_died:
            yield opera.play
