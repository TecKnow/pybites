...
def extract_course_times(course=COURSE):
    """Return the course timings from the passed in
       ...
       Return this list.
    """
    course_time_re = re.compile(r"\d\d:\d\d")
    return [m.group() for m in course_time_re.finditer(course)]
...
def get_all_hashtags_and_links(tweet=TWEET):
    ...
    """
    tweet_re = re.compile(
        r"((?P<hashtag>#\w*)|(?:\s)(?P<url>\S+://\S+)(?:\s*))")
    return [m.group('hashtag') or m.group('url') for m in tweet_re.finditer(tweet)]
...
def match_first_paragraph(html=HTML):
    """Extract the first paragraph of the passed in
       ...
       Return this string.
    """
    html_re = re.compile(r"<p>(?P<ptext>.*?)</p>")
    for m in html_re.finditer(html):
        return m.group("ptext")
