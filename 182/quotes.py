...
def extract_quotes(html: str = HTML) -> dict:
    ...
    return {match.group("author"): match.group("quote") for line in html.splitlines() if (match := extract_quotes.re.match(line)) is not None}


extract_quotes.re = re.compile(
    r'\s*<p>\d+.\s*"(?P<quote>[^"]*?)"\s*-\s(?P<author>[^<]*)</p>\s*')
