...
def filter_tweets_on_polarity(tweets: List[Tweet], keep_positive: bool = True) -> List[Tweet]:
    ...
    return [tweet for tweet in tweets if tweet.polarity > 0 and keep_positive or tweet.polarity < 0 and not keep_positive]


def order_tweets_by_polarity(tweets: List[Tweet], positive_highest: bool = True) -> List[Tweet]:
    """Sort the tweets by polarity, receives positive_highest which determines
       the order. Returns a list of ordered tweets."""
    return sorted(tweets, key=lambda t: t.polarity,  reverse=True if positive_highest else False)
