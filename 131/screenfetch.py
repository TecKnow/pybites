...
def sysinfo_scrape(output: str) -> Dict[str, str]:
    ...
    lines_without_logo = [ rline for line in output.splitlines() if (rline:=line[line.rfind("  "):].strip())]
    user_name, *data_lines = lines_without_logo
    data_lines.insert(0, f"Name: {user_name}")
    return dict(((key.strip(), value.strip()) for key, value in (data_line.split(":", 1) for data_line in data_lines)))

if __name__== "__main__":
    from pprint import pprint 
    pprint(sysinfo_scrape(output))