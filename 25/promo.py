...
class Promo:
    ...
    def _pick_random_bite(self):
        ...
        all_bites_set = set(self.all_bites.keys())
        remaining_bites_set = all_bites_set.difference(self.bites_done)
        if not remaining_bites_set:
            raise NoBitesAvailable()
        return random.choice(tuple(remaining_bites_set))

    def new_bite(self):
        """Get  a random Bite using _pick_random_bite,
           add it to self.bites_done, then return it"""
        selected_bite = self._pick_random_bite()
        self.bites_done.add(selected_bite)
        return selected_bite
