...
# I added this test because I wanted to be sure that methods like setdefault() would call __setitem_()
def test_setdefault(bd, capfd):
    # if same day and month, but different year = match
    bd.setdefault("dave", date(1982, 7, 15))   # same as tim, except year
    output = capfd.readouterr()[0].strip()
    assert output == MSG.format('dave')