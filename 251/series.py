...
def float_series() -> pd.Series:
    ...
    """
    return pd.Series(data=[0.001*x for x in range(1001)])
...
def alpha_index_series() -> pd.Series:
    ...
    """
    return pd.Series(data=dict(zip(string.ascii_lowercase, range(1, 27))))
...
def object_values_series() -> pd.Series:
    ...
    Don't worry about the series name.
    """
    return pd.Series(data=dict(zip(range(101, 127), string.ascii_uppercase)))
